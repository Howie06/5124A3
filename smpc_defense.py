...
# Import CrypTen for SMPC
import crypten
import crypten.nn as cnn
import crypten.mpc as mpc

from mnist import MNIST  # Assuming MNIST model definition is available as mnist.py
...
# ----------------------------------------
# Secure one-hot posterior computation
# ----------------------------------------
def get_secure_one_hot_posteriors(model_enc, loader, device):
    """
    Run SMPC inference on `loader` using encrypted model `model_enc`.
    Returns one-hot posterior arrays (num_samples, 10) and true labels.
    """
    all_one_hots = []
    all_labels = []
    for data, target in loader:
        # Secret-share input among parties (party 0 holds data)
        data_enc = crypten.cryptensor(data, src=0)

        # Encrypted forward pass
        logits_enc = model_enc(data_enc)
        # Compute encrypted softmax
        probs_enc = logits_enc.softmax(dim=1)

        # Secure argmax to get predicted label index (still encrypted)
        pred_label_enc = probs_enc.argmax(dim=1)

        # Reveal the label index to all parties
        pred_label = pred_label_enc.get_plain_text().cpu()

        # Create one-hot vectors
        one_hot = torch.zeros((len(target), 10))
        one_hot[range(len(target)), pred_label.long()] = 1.0

        all_one_hots.append(one_hot.numpy())
        all_labels.append(target.numpy())
    return np.concatenate(all_one_hots, axis=0), np.concatenate(all_labels, axis=0)

# ----------------------------------------
# Main execution under SMPC
# ----------------------------------------
...
def main():
    ...
    # ----------------------------------------------
    # Convert to CrypTen Encrypted Model
    # ----------------------------------------------
    # Create a dummy input for tracing
    dummy_input = torch.randn(1, 1, 28, 28).to(device)
    # Wrap PyTorch model into CrypTen model
    ct_model = cnn.from_pytorch(target_model, dummy_input)
    # Encrypt (secret-share) the model parameters
    ct_model.encrypt()
    ...
    # -------------------------------------------------
    # Final Evaluation on Target Model Using SMPC Defense
    # -------------------------------------------------
    ...
    # Gather one-hot outputs & true labels for members using secure inference
    post_mem, labels_mem = get_secure_one_hot_posteriors(ct_model, member_loader, device)
    # Gather one-hot outputs & true labels for non-members using secure inference
    post_nonmem, labels_nonmem = get_secure_one_hot_posteriors(ct_model, nonmember_loader, device)
    ...
    print("=== Final Membership Inference Attack Results Under SMPC Defense ===")
